...
def resize(data: List[np.ndarray], size: Optional[List[int]] = None) -> List[np.ndarray]:
    ...
    return [cv.resize(d, size[::-1]) for d in data]


def make_square_grid(data: List[np.ndarray], values: Optional[int] = None) -> np.ndarray:
    values = np.min(data) if not values else values
    data = resize(data)
